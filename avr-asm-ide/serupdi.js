...
async function serupdi_init(){
  ...
  ser.close = async function(x){
    ser.reader.releaseLock();
    ser.writer.releaseLock();
    ser.port.close();
    ser.port = null;
    _log(1, 'port closed.');
  }
...
async function updi_enter(){
  ...
  // _log(1,await updi_lds(0x1100));
  // _log(1,await updi_lds(0x1101));
  // _log(1,await updi_lds(0x1102));
