...
let presets = {
  'juicy':
`squiggy.tape_brush(o=>({w:Math.min(Math.max(20-o.z*0.5,5),20)}),{cap:'round',join:'round'})`,
  'round':
`squiggy.tape_brush(o=>({w:30}),{cap:'round',join:'round'})`,
  'thin':
`squiggy.tape_brush(o=>({w:4}),{cap:'round',join:'round'})`,
  'slug':
`squiggy.tape_brush(o=>({w:Math.sin(o.t*Math.PI)*30+2}),{cap:'round',join:'round'})`,
  'gut':
`squiggy.tape_brush(o=>({w:noise(o.x*0.01,o.y*0.01)*80+1}),{cap:'round',join:'round'})`,
  'slash45':
...
ta.style='resize:none;position:absolute;left:1px;top:630px;width:650px;height:200px;border:none;outline:1px solid black;margin:0px;';
